...
// descender; in other words, it's equivalent to calling  
// and computing:
...
int stbtt_InitFont(stbtt_fontinfo *info, const unsigned char *data2, int fontstart)
{
   ...
   cmap = stbtt__find_table(data, fontstart, (char*)"cmap");
   info->loca = stbtt__find_table(data, fontstart, (char*)"loca");
   info->head = stbtt__find_table(data, fontstart, (char*)"head");
   info->glyf = stbtt__find_table(data, fontstart, (char*)"glyf");
   info->hhea = stbtt__find_table(data, fontstart, (char*)"hhea");
   info->hmtx = stbtt__find_table(data, fontstart, (char*)"hmtx");
   if (!cmap || !info->loca || !info->head || !info->glyf || !info->hhea || !info->hmtx)
      return 0;

   t = stbtt__find_table(data, fontstart, (char*)"maxp");
   if (t)
...
char *stbtt_GetFontNameString(const stbtt_fontinfo *font, int *length, int platformID, int encodingID, int languageID, int nameID)
{
   ...
   stbtt_uint32 nm = stbtt__find_table(fc, offset, (char*)"name");
   if (!nm) return NULL;
...
static int stbtt__matches(stbtt_uint8 *fc, stbtt_uint32 offset, stbtt_uint8 *name, stbtt_int32 flags)
{
   ...
   if (flags) {
      hd = stbtt__find_table(fc, offset, (char*)"head");
      if ((ttUSHORT(fc+hd+44) & 7) != (flags & 7)) return 0;
   }

   nm = stbtt__find_table(fc, offset, (char*)"name");
   if (!nm) return 0;
